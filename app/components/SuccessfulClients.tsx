...
export default function SuccessfulClients() {
  ...
  return (
    <section className="w-full bg-[#CAD4DC] my-16 py-16 max-w-8xl mx-auto px-6 md:px-8 text-center">
      <h2 className="text-3xl md:text-4xl font-bold font-sans-pro text-[#0A3D62] mb-10">
        Our Successful Clients
      </h2>

      <div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 md:gap-2 items-center justify-between px-2 md:px-8 max-w-7xl mx-auto">
          {clients.map((client) => (
            <div key={client.name} className="flex flex-col items-center">
              <Image 
                src={client.logo} 
                alt={client.name} 
                width={client.name === "Client F" ? 80 : 120} 
                height={client.name === "Client F" ? 60 : 100} 
                className="mb-4" 
              />
            </div>
          ))}
